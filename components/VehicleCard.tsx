...
const SpecIcon: React.FC<{ icon: React.ReactNode, text: string }> = ({ icon, text }) => (
    <div className="flex items-center gap-1.5 text-xs sm:text-sm text-brand-gray-600 dark:text-brand-text">
        {icon}
        <span>{text}</span>
    </div>
);
...
const VehicleCard: React.FC<VehicleCardProps> = ({ vehicle, onSelect, onToggleCompare, isSelectedForCompare, onToggleWishlist, isInWishlist, isCompareDisabled, onViewSellerProfile, onQuickView }) => {
  ...
  return (
    <div 
      ...
      style={{ ['--hover-ring-color' as any]: 'var(--brand-turquoise)' }}
    >
      <div className="relative overflow-hidden">
        ...
        {vehicle.isFeatured && (
          <div className="absolute top-3 left-3 text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1" style={{ background: 'var(--gradient-warm)' }}>
         ...
         <div className="absolute top-3 right-3 flex items-center gap-2 z-10">
            <button
              ...
              style={{ backgroundColor: isSelectedForCompare ? 'var(--brand-turquoise)' : 'rgba(0,0,0,0.3)' }}
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 transition-colors" style={{ color: isInWishlist ? 'var(--brand-mint)' : 'white' }} viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" />
              </svg>
            </button>
        </div>
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
             <button onClick={handleQuickViewClick} className="bg-brand-white/90 dark:bg-black/80 text-brand-text-dark dark:text-white font-bold py-2 px-6 rounded-full transform hover:scale-105 transition-transform backdrop-blur-sm">
                Quick View
            </button>
        </div>
      </div>
      <div className="p-3 sm:p-5 flex-grow flex flex-col">
        <div className="flex justify-between items-start">
            <h3 className="text-base leading-tight sm:text-xl sm:leading-normal font-bold text-brand-text-dark dark:text-brand-text-dark">{vehicle.make} {vehicle.model} {vehicle.variant || ''}</h3>
            <span className="text-sm sm:text-lg font-semibold text-brand-text dark:text-brand-text bg-brand-gray-100 dark:bg-brand-gray-700 px-2 py-0.5 rounded">{vehicle.year}</span>
        </div>
        
        <div className="mt-2 text-xs text-brand-text dark:text-brand-text truncate">
        ...
        <div className="mt-3 pt-3 sm:mt-4 sm:pt-4 border-t border-brand-gray-100 dark:border-brand-gray-200 grid grid-cols-2 gap-x-2 gap-y-1.5 sm:gap-x-4 sm:gap-y-2">
            <SpecIcon icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" style={{ color: 'var(--brand-mint)' }} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.414-1.414L11 10.586V6z" clipRule="evenodd" /></svg>} text={`${vehicle.mileage.toLocaleString('en-IN')} kms`} />
            ...
            <SpecIcon icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" style={{ color: 'var(--brand-mint)' }} viewBox="0 0 20 20" fill="currentColor"><path d="M5 4a1 1 0 00-2 0v7.268a2 2 0 000 3.464V16a1 1 0 102 0v-1.268a2 2 0 000-3.464V4zM11 4a1 1 0 10-2 0v1.268a2 2 0 000 3.464V16a1 1 0 102 0V8.732a2 2 0 000-3.464V4zM16 3a1 1 0 011 1v7.268a2 2 0 010 3.464V16a1 1 0 11-2 0v-1.268a2 2 0 010-3.464V4a1 1 0 011-1z" /></svg>} text={vehicle.transmission} />
            <SpecIcon icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" style={{ color: 'var(--brand-mint)' }} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" /></svg>} text={`${vehicle.city}, ${vehicle.state}`} />
        </div>

        <div className="mt-auto pt-3 sm:pt-4 flex justify-between items-center">
             <p className="text-xl sm:text-2xl font-extrabold" style={{ color: 'var(--brand-turquoise)' }}>₹{vehicle.price.toLocaleString('en-IN')}</p>
             <div className="flex items-center gap-2">
                <StarRating rating={vehicle.averageRating || 0} readOnly size="sm"/>
                <span className="text-xs text-brand-text dark:text-brand-text">({vehicle.ratingCount || 0})</span>
            </div>
        </div>
      </div>
    </div>

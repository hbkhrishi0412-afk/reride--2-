...
const VehicleCard: React.FC<VehicleCardProps> = ({ vehicle, onSelect, onToggleCompare, isSelectedForCompare, onToggleWishlist, isInWishlist, isCompareDisabled, onViewSellerProfile, onQuickView }) => {
  ...
  return (
    <div 
      ...
      style={{ ['--hover-ring-color' as any]: 'var(--brand-teal)' }}
    >
      <div className="relative overflow-hidden">
        ...
        {vehicle.isFeatured && (
          <div className="absolute top-3 left-3 text-brand-cream text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1" style={{ background: 'var(--gradient-warm)' }}>
         ...
         <div className="absolute top-3 right-3 flex items-center gap-2 z-10">
            <button
              ...
              className={`p-2 rounded-full backdrop-blur-sm text-brand-cream hover:bg-black/50 transition-colors ${isCompareDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
              style={{ backgroundColor: isSelectedForCompare ? 'var(--brand-teal)' : 'rgba(0,0,0,0.3)' }}
            ...
            <button
              ...
              className="p-2 rounded-full bg-black/30 backdrop-blur-sm text-brand-cream hover:bg-black/50 transition-colors"
              aria-label={isInWishlist ? "Remove from wishlist" : "Add to wishlist"}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 transition-colors" style={{ color: isInWishlist ? 'var(--brand-gold)' : 'white' }} viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" />
              </svg>
            </button>
        </div>
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
             <button onClick={handleQuickViewClick} className="bg-brand-cream/90 dark:bg-black/80 text-brand-gray-800 dark:text-brand-cream font-bold py-2 px-6 rounded-full transform hover:scale-105 transition-transform backdrop-blur-sm">
                Quick View
            </button>
        </div>
      </div>
      <div className="p-3 sm:p-5 flex-grow flex flex-col">
        ...
        <div className="mt-2 text-xs text-brand-gray-500 dark:text-brand-gray-500 truncate">
           By: <button onClick={handleSellerClick} className="font-semibold hover:underline focus:outline-none transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}>{vehicle.sellerName}</button>
        </div>
        ...
        <div className="mt-3 pt-3 sm:mt-4 sm:pt-4 border-t border-brand-gray-100 dark:border-brand-gray-700 grid grid-cols-2 gap-x-2 gap-y-1.5 sm:gap-x-4 sm:gap-y-2">
            <SpecIcon icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" style={{ color: 'var(--brand-gold)' }} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.414-1.414L11 10.586V6z" clipRule="evenodd" /></svg>} text={`${vehicle.mileage.toLocaleString('en-IN')} kms`} />
            <SpecIcon icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" style={{ color: 'var(--brand-gold)' }} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5 2a1 1 0 011 1v1h1a1 1 0 010 2H6v1a1 1 0 01-2 0V6H3a1 1 0 010-2h1V3a1 1 0 011-1zm0 10a1 1 0 011 1v1h1a1 1 0 110 2H6v1a1 1 0 11-2 0v-1H3a1 1 0 110-2h1v-1a1 1 0 011-1zM12 2a1 1 0 01.967.744L14.146 7.2 17.5 9.134a1 1 0 010 1.732l-3.354 1.935-1.18 4.455a1 1 0 01-1.933 0L9.854 12.8 6.5 10.866a1 1 0 010-1.732l3.354-1.935 1.18-4.455A1 1 0 0112 2z" clipRule="evenodd" /></svg>} text={vehicle.fuelType} />
            <SpecIcon icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" style={{ color: 'var(--brand-gold)' }} viewBox="0 0 20 20" fill="currentColor"><path d="M5 4a1 1 0 00-2 0v7.268a2 2 0 000 3.464V16a1 1 0 102 0v-1.268a2 2 0 000-3.464V4zM11 4a1 1 0 10-2 0v1.268a2 2 0 000 3.464V16a1 1 0 102 0V8.732a2 2 0 000-3.464V4zM16 3a1 1 0 011 1v7.268a2 2 0 010 3.464V16a1 1 0 11-2 0v-1.268a2 2 0 010-3.464V4a1 1 0 011-1z" /></svg>} text={vehicle.transmission} />
            <SpecIcon icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" style={{ color: 'var(--brand-gold)' }} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" /></svg>} text={`${vehicle.city}, ${vehicle.state}`} />
        </div>

        <div className="mt-auto pt-3 sm:pt-4 flex justify-between items-center">
             <p className="text-xl sm:text-2xl font-extrabold" style={{ color: 'var(--brand-teal)' }}>₹{vehicle.price.toLocaleString('en-IN')}</p>

            ...
            <div className="container mx-auto px-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
                    {plans.map((plan) => (
                        <div
                            key={plan.id}
                            className={`bg-white dark:bg-brand-gray-800 rounded-2xl shadow-soft-lg p-8 flex flex-col transition-transform duration-300 ${plan.isMostPopular ? 'transform lg:scale-105 border-2 shadow-glow' : ''}`} style={plan.isMostPopular ? { borderColor: 'var(--brand-deep-red)' } : undefined}
                        >
                            {plan.isMostPopular && (

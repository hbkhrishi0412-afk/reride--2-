        ...
        <div className="bg-brand-gray-50 dark:bg-brand-gray-dark py-12 sm:py-20 animate-fade-in">
            <div className="container mx-auto px-4">
                <div className="max-w-2xl mx-auto text-center mb-12">
                    <h1 className="text-4xl md:text-5xl font-extrabold text-brand-gray-900 dark:text-brand-cream">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
                    {plans.map((plan) => (
                        <div
                            key={plan.id}
                            className={`bg-brand-cream dark:bg-brand-gray-800 rounded-2xl shadow-soft-lg p-8 flex flex-col transition-transform duration-300 ${plan.isMostPopular ? 'transform lg:scale-105 border-2 shadow-glow' : ''}`} style={plan.isMostPopular ? { borderColor: 'var(--brand-teal)' } : undefined}
                        >
                            {plan.isMostPopular && (
                                <div className="absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2">
                                    <span className="text-brand-cream text-xs font-bold px-4 py-1 rounded-full uppercase" style={{ background: 'var(--brand-teal)' }}>
                                        Most Popular
                                    </span>
                                </div>
                            )}
                            <h3 className="text-2xl font-bold text-brand-gray-900 dark:text-brand-cream">{plan.name}</h3>
                            <p className="mt-4 text-4xl font-extrabold text-brand-gray-900 dark:text-brand-cream">
                            ...
                            <button
                                ...
                                className={`mt-10 w-full font-bold py-3 px-6 rounded-lg text-lg transition-colors disabled:opacity-70 disabled:cursor-not-allowed
                                    ${currentPlanId === plan.id
                                        ...
                                        : plan.isMostPopular
                                            ? 'btn-brand-primary text-brand-cream'
                                            : 'btn-brand-soft text-brand-cream'
                                    }`}
                            >

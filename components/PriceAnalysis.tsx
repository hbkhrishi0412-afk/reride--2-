...
const PriceAnalysis: React.FC<PriceAnalysisProps> = ({ vehicle, similarVehicles }) => {
  ...
  return (
    <div className="bg-brand-white dark:bg-brand-gray-800 rounded-xl shadow-soft-lg p-6">
      ...
      {!analysis && !isLoading && !error && (
        <>
          ...
          <button
            ...
            className="w-full bg-brand-deep-red text-brand-white font-bold py-3 px-6 rounded-lg text-lg hover:bg-brand-deep-red transition-all transform hover:scale-105 flex items-center justify-center gap-2"
          >
      ...
      {error && (
        <div className="text-center p-4 bg-brand-deep-red-light dark:bg-brand-deep-red/20 text-brand-deep-red dark:text-brand-deep-red rounded-lg">
            <p className="font-semibold">Analysis Failed</p>
            <p className="text-sm">{error}</p>
            <button onClick={handleAnalyzePrice} className="mt-2 text-sm font-bold text-brand-deep-red hover:underline">Try Again</button>
        </div>
      )}
      ...
      {analysis && (
        <div className="animate-fade-in space-y-4">
            <div>
                ...
                <p className="text-2xl font-extrabold text-brand-deep-red dark:text-brand-deep-red">

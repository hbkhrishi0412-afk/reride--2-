...
const CheckIcon: React.FC = () => (
    <svg className="w-6 h-6 text-brand-deep-red mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
);

const XIcon: React.FC = () => (
    <svg className="w-6 h-6 text-brand-deep-red mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
);
...
const Comparison: React.FC<ComparisonProps> = ({ vehicles, onBack: onBackToHome, onToggleCompare }) => {
  ...
  if (vehicles.length === 0) {
    return (
      <div className="text-center py-20 bg-brand-white dark:bg-brand-gray-800 rounded-xl shadow-soft-lg">
        ...
        <button onClick={onBackToHome} className="mt-6 btn-brand-primary text-brand-white font-bold py-2 px-6 rounded-lg transition-colors">
  ...
  return (
    <div className="bg-brand-white dark:bg-brand-gray-800 p-6 rounded-xl shadow-soft-lg animate-fade-in">
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b-2 border-brand-gray-300 dark:border-brand-gray-600">
              <th className="text-left font-bold text-lg text-brand-gray-700 p-4 sticky left-0 bg-brand-white dark:bg-brand-gray-800 z-10">Feature</th>
              {vehicles.map(vehicle => (
                <th key={vehicle.id} className="p-4 min-w-[220px]">
                  ...
                  <button onClick={() => onToggleCompare(vehicle.id)} className="mt-2 text-sm text-brand-deep-red hover:text-brand-deep-red">Remove</button>
                </th>
              ))}
            </tr>
          </thead>
            ...
            {specFields.map((key) => {
              ...
              return (
                <tr key={String(key)} className="border-b border-brand-gray-200 dark:border-brand-gray-700">
                  <td className="font-semibold text-brand-gray-600 dark:text-brand-gray-300 p-4 sticky left-0 bg-brand-white dark:bg-brand-gray-800 z-10">{specLabels[key]}</td>
                  {vehicles.map(vehicle => {
                    ...
                    const cellStyle = highlightDiffs && hasDifference ? { backgroundColor: 'var(--brand-orange-light)' } : undefined;
                    return (
                      <td key={`${vehicle.id}-${String(key)}`} className={`p-4 text-center dark:text-brand-gray-200 transition-colors ${cellClass} ${isBest ? 'bg-brand-deep-red-light dark:bg-brand-deep-red/20' : ''}`}>
                         <span className={`inline-flex items-center gap-2 ${isBest ? 'font-bold text-brand-deep-red dark:text-brand-deep-red' : ''}`}>
                            {(() => {
                            ...
                            {isBest && <span className="text-xs font-semibold bg-brand-deep-red text-brand-deep-red px-2 py-0.5 rounded-full">Best</span>}
                         </span>
                      </td>
                    )
                  })}
            ...
            {allFeatures.map((feature) => {
              ...
              return (
                 <tr key={feature} className="border-b border-brand-gray-200 dark:border-brand-gray-700">
                     <td className="font-semibold text-brand-gray-600 dark:text-brand-gray-300 p-4 sticky left-0 bg-brand-white dark:bg-brand-gray-800 z-10">{feature}</td>
                     {vehicles.map(vehicle => (
                        <td key={`${vehicle.id}-${feature}`} className="p-4 transition-colors" style={highlightDiffs && hasDifference ? { backgroundColor: 'var(--brand-orange-light)' } : undefined}>

...
const CheckIcon: React.FC = () => (
    <svg className="w-6 h-6 text-brand-teal mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
);

const XIcon: React.FC = () => (
    <svg className="w-6 h-6 text-brand-teal mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
);
...
const Comparison: React.FC<ComparisonProps> = ({ vehicles, onBack: onBackToHome, onToggleCompare }) => {
  ...
  if (vehicles.length === 0) {
    return (
      <div className="text-center py-20 bg-brand-cream dark:bg-brand-gray-800 rounded-xl shadow-soft-lg">
        ...
        <button onClick={onBackToHome} className="mt-6 btn-brand-primary text-brand-cream font-bold py-2 px-6 rounded-lg transition-colors">
  ...
  return (
    <div className="bg-brand-cream dark:bg-brand-gray-800 p-6 rounded-xl shadow-soft-lg animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            <h1 className="text-3xl font-extrabold text-brand-gray-900 dark:text-brand-gray-100">Compare Vehicles</h1>
            <button onClick={onBackToHome} className="text-sm font-medium hover:underline mt-1 transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}>
                &larr; Back to Listings
            </button>
        </div>
        <div className="flex items-center space-x-3 bg-brand-gray-100 dark:bg-brand-gray-700 p-2 rounded-lg">
          ...
          <button onClick={() => setHighlightDiffs(!highlightDiffs)} id="highlight-toggle" className={`${highlightDiffs ? '' : 'bg-brand-gray-300 dark:bg-brand-gray-600'} relative inline-flex items-center h-6 rounded-full w-11 transition-colors`} style={highlightDiffs ? { background: 'var(--brand-teal)' } : undefined}>
              <span className={`${highlightDiffs ? 'translate-x-6' : 'translate-x-1'} inline-block w-4 h-4 transform bg-brand-cream rounded-full transition-transform`} />
          </button>
        </div>
      </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b-2 border-brand-gray-300 dark:border-brand-gray-600">
              <th className="text-left font-bold text-lg text-brand-gray-700 p-4 sticky left-0 bg-brand-cream dark:bg-brand-gray-800 z-10">Feature</th>
              {vehicles.map(vehicle => (
                <th key={vehicle.id} className="p-4 min-w-[220px]">
                  ...
                  <button onClick={() => onToggleCompare(vehicle.id)} className="mt-2 text-sm text-brand-teal hover:text-brand-teal">Remove</button>
                </th>
              ))}
            </tr>
          </thead>
            ...
            {specFields.map((key) => {
              ...
              return (
                <tr key={String(key)} className="border-b border-brand-gray-200 dark:border-brand-gray-700">
                  <td className="font-semibold text-brand-gray-600 dark:text-brand-gray-300 p-4 sticky left-0 bg-brand-cream dark:bg-brand-gray-800 z-10">{specLabels[key]}</td>
                  {vehicles.map(vehicle => {
                    ...
                    const cellStyle = highlightDiffs && hasDifference ? { backgroundColor: 'var(--brand-gold-light)' } : undefined;
                    return (
                      <td key={`${vehicle.id}-${String(key)}`} className={`p-4 text-center dark:text-brand-gray-200 transition-colors ${cellClass} ${isBest ? 'bg-brand-teal-light dark:bg-brand-teal/20' : ''}`}>
                         <span className={`inline-flex items-center gap-2 ${isBest ? 'font-bold text-brand-teal dark:text-brand-teal' : ''}`}>
                            {(() => {
                            ...
                            {isBest && <span className="text-xs font-semibold bg-brand-teal text-brand-teal px-2 py-0.5 rounded-full">Best</span>}
                         </span>
                      </td>
                    )
                  })}
            ...
            {allFeatures.map((feature) => {
              ...
              return (
                 <tr key={feature} className="border-b border-brand-gray-200 dark:border-brand-gray-700">
                     <td className="font-semibold text-brand-gray-600 dark:text-brand-gray-300 p-4 sticky left-0 bg-brand-cream dark:bg-brand-gray-800 z-10">{feature}</td>
                     {vehicles.map(vehicle => (
                        <td key={`${vehicle.id}-${feature}`} className="p-4 transition-colors" style={highlightDiffs && hasDifference ? { backgroundColor: 'var(--brand-gold-light)' } : undefined}>

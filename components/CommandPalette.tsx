...
const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose, onNavigate, currentUser, onLogout }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[100] flex items-start justify-center pt-20" onClick={onClose}>
      <div className="bg-brand-cream dark:bg-brand-gray-800 rounded-lg shadow-2xl w-full max-w-xl animate-fade-in" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-brand-gold dark:border-gray-700 flex items-center gap-3">
          {ICONS.SEARCH}
          <input
            ref={inputRef}
            type="text"
            ...
            className="w-full bg-transparent focus:outline-none text-lg text-brand-slate dark:text-brand-slate"
          />
        </div>
        <div className="max-h-96 overflow-y-auto">
          {Object.entries(groupedCommands).map(([section, commands]) => (
            <div key={section} className="p-2">
              <h3 className="text-xs font-semibold text-brand-slate dark:text-brand-slate px-2 mb-1">{section}</h3>
              <ul>
                  ...
                  return (
                    <li key={command.id}>
                      <button
                        ...
                        className={`w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-md ${activeIndex === globalIndex ? 'text-brand-cream' : 'text-brand-slate dark:text-brand-slate hover:bg-brand-cream dark:hover:bg-brand-cream'}`} style={activeIndex === globalIndex ? { background: 'var(--gradient-primary)' } : undefined}
                      >
                        <span className={activeIndex === globalIndex ? 'text-brand-cream' : 'text-brand-slate dark:text-brand-slate'}>{command.icon}</span>
          ...
          {filteredCommands.length === 0 && (
            <p className="p-4 text-center text-brand-slate">No commands found.</p>
          )}
        </div>
      </div>
    </div>

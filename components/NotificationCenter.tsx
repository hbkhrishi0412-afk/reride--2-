...
const NotificationIcon: React.FC<{ type: Notification['targetType'] }> = ({ type }) => {
    ...
    switch(type) {
        case 'conversation':
            return <svg xmlns="http://www.w3.org/2000/svg" className={baseClass} style={{ color: 'var(--brand-orange)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>;
        case 'price_drop':
             return <svg xmlns="http://www.w3.org/2000/svg" className={`${baseClass} text-brand-deep-red`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>;
        case 'insurance_expiry':
        case 'general_admin':
        case 'vehicle':
             return <svg xmlns="http://www.w3.org/2000/svg" className={`${baseClass} text-brand-blackcurrant`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>;
        default:
            return <svg xmlns="http://www.w3.org/2000/svg" className={`${baseClass} text-brand-blackcurrant`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
    }
}
...
const NotificationCenter: React.FC<NotificationCenterProps> = ({ notifications, onNotificationClick, onMarkAllAsRead }) => {
    ...
    return (
        <div className="absolute top-full right-0 mt-2 w-80 md:w-96 bg-brand-white dark:bg-brand-gray-800 rounded-lg shadow-2xl border dark:border-brand-gray-700 animate-fade-in flex flex-col max-h-[70vh]">
            <header className="p-3 border-b dark:border-brand-gray-700 flex justify-between items-center">
                <h3 className="font-bold text-lg text-brand-gray-900 dark:text-brand-white">Notifications</h3>
                {unreadCount > 0 && (
            ...
            <div className="overflow-y-auto">
                {notifications.length > 0 ? (
                    ...
                    <div className="p-8 text-center">
                        ...
                        <h4 className="mt-2 text-sm font-semibold text-brand-blackcurrant dark:text-brand-blackcurrant">All caught up!</h4>
                        <p className="mt-1 text-xs text-brand-blackcurrant dark:text-brand-blackcurrant">You have no new notifications.</p>
                    </div>
                )}
            </div>

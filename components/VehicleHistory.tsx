...
const TimelineIcon: React.FC<{ type: 'service' | 'accident' }> = ({ type }) => {
  ...
  if (type === 'service') {
    return (
      <div className={`${baseClasses}`} style={{ backgroundColor: 'var(--brand-gold-light)', color: 'var(--brand-teal)' }}>
  ...
  return (
    <div className={`${baseClasses} bg-brand-teal-light text-brand-teal`}>
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
const VehicleHistory: React.FC<VehicleHistoryProps> = ({ serviceRecords, accidentHistory }) => {
  ...
  if (combinedHistory.length === 0) {
    return (
      <div className="p-6 bg-brand-cream dark:bg-brand-gray-800 rounded-xl shadow-soft">
  ...
  return (
    <div className="p-6 bg-brand-cream dark:bg-brand-gray-800 rounded-xl shadow-soft">
      ...
      <ol className="relative border-l border-brand-gold dark:border-gray-700 ml-5">
          ...
          <li key={index} className="mb-10 ml-8">
            ...
            <div className="p-4 bg-brand-gray-50 dark:bg-brand-gray-900/50 border border-brand-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
                <div className="flex justify-between items-center mb-2">
                    <time className="text-sm font-normal leading-none text-brand-slate dark:text-brand-slate">{new Date(item.date).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</time>
                    {item.type === 'accident' && <span className="bg-brand-teal-light text-brand-teal text-xs font-semibold px-2.5 py-0.5 rounded dark:bg-brand-teal dark:text-brand-teal">{item.severity}</span>}
                </div>
                <h4 className="text-lg font-semibold text-brand-slate dark:text-brand-cream">
                    {item.type === 'service' ? item.service : 'Accident Reported'}
                </h4>
                <p className="text-base font-normal text-brand-slate dark:text-brand-slate">

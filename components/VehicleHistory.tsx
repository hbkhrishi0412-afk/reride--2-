...
const TimelineIcon: React.FC<{ type: 'service' | 'accident' }> = ({ type }) => {
  ...
  if (type === 'service') {
    return (
      <div className={`${baseClasses}`} style={{ backgroundColor: 'var(--brand-orange-light)', color: 'var(--brand-deep-red)' }}>
  ...
  return (
    <div className={`${baseClasses} bg-brand-deep-red-light text-brand-deep-red`}>
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
const VehicleHistory: React.FC<VehicleHistoryProps> = ({ serviceRecords, accidentHistory }) => {
  ...
  if (combinedHistory.length === 0) {
    return (
      <div className="p-6 bg-brand-white dark:bg-brand-gray-800 rounded-xl shadow-soft">
  ...
  return (
    <div className="p-6 bg-brand-white dark:bg-brand-gray-800 rounded-xl shadow-soft">
      ...
      <ol className="relative border-l border-brand-orange dark:border-gray-700 ml-5">
          ...
          <li key={index} className="mb-10 ml-8">
            ...
            <div className="p-4 bg-brand-gray-50 dark:bg-brand-gray-900/50 border border-brand-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
                <div className="flex justify-between items-center mb-2">
                    <time className="text-sm font-normal leading-none text-brand-blackcurrant dark:text-brand-blackcurrant">{new Date(item.date).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</time>
                    {item.type === 'accident' && <span className="bg-brand-deep-red-light text-brand-deep-red text-xs font-semibold px-2.5 py-0.5 rounded dark:bg-brand-deep-red dark:text-brand-deep-red">{item.severity}</span>}
                </div>
                <h4 className="text-lg font-semibold text-brand-blackcurrant dark:text-brand-white">
                    {item.type === 'service' ? item.service : 'Accident Reported'}
                </h4>
                <p className="text-base font-normal text-brand-blackcurrant dark:text-brand-blackcurrant">

...
const KeySpec: React.FC<{ label: string; value: string | number; }> = memo(({ label, value }) => (
    <div className="flex justify-between items-baseline py-2 border-b border-brand-gray-100 dark:border-brand-gray-200">
        ...
        <span className="font-bold text-brand-gray-900 dark:text-brand-text-dark text-right">{value}</span>
    </div>
));
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[100] p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col md:flex-row overflow-hidden relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-2 right-2 text-white md:text-brand-text bg-black/30 md:bg-transparent rounded-full w-8 h-8 flex items-center justify-center hover:text-brand-text-dark dark:hover:text-white z-10 text-2xl">&times;</button>
          ...
          <div className="grid grid-cols-5 gap-2 mt-2">
            {vehicle.images.slice(0, 5).map((img, index) => (
              <img
                ...
                className={`cursor-pointer rounded-md border-2 h-16 w-full object-cover ${mainImage === img ? '' : 'border-transparent'} transition`} style={mainImage === img ? { borderColor: 'var(--brand-turquoise)' } : undefined} onMouseEnter={(e) => mainImage !== img && (e.currentTarget.style.borderColor = 'var(--brand-mint)')} onMouseLeave={(e) => mainImage !== img && (e.currentTarget.style.borderColor = '')}
        ...
        <div className="w-full md:w-1/2 p-6 flex flex-col overflow-y-auto">
          <div>
            <h2 className="text-3xl font-bold text-brand-gray-900 dark:text-brand-text-dark">{vehicle.year} {vehicle.make} {vehicle.model}</h2>
            <p className="text-brand-text dark:text-brand-text">{vehicle.variant || ''}</p>
            <p className="text-3xl font-extrabold my-4" style={{ color: 'var(--brand-turquoise)' }}>₹{vehicle.price.toLocaleString('en-IN')}</p>
          </div>
          ...
          <div className="mt-auto pt-6 space-y-3">
            ...
            <div className="flex gap-3">
                <button
                    ...
                    className={`w-full font-bold py-3 px-6 rounded-lg text-lg transition-all flex items-center justify-center gap-2 ${isInWishlist ? 'bg-brand-mint text-brand-turquoise' : 'bg-brand-gray-200 dark:bg-brand-gray-700'}`}
                >
                 ...
                 <button
                    ...
                    className={`w-full font-bold py-3 px-6 rounded-lg text-lg transition-all ${isComparing ? 'bg-brand-turquoise text-brand-turquoise' : 'bg-brand-gray-200 dark:bg-brand-gray-700'} ${isCompareDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                    Compare
                </button>
            </div>

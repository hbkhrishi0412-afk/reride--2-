...
const QuickViewModal: React.FC<QuickViewModalProps> = ({ vehicle, onClose, onSelectVehicle, onToggleCompare, onToggleWishlist, comparisonList, wishlist }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[100] p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-brand-white dark:bg-brand-gray-800 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col md:flex-row overflow-hidden relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-2 right-2 text-brand-white md:text-brand-gray-500 bg-black/30 md:bg-transparent rounded-full w-8 h-8 flex items-center justify-center hover:text-brand-gray-800 dark:hover:text-brand-white z-10 text-2xl">&times;</button>
          ...
          <div className="grid grid-cols-5 gap-2 mt-2">
            {vehicle.images.slice(0, 5).map((img, index) => (
              <img
                ...
                className={`cursor-pointer rounded-md border-2 h-16 w-full object-cover ${mainImage === img ? '' : 'border-transparent'} transition`} style={mainImage === img ? { borderColor: 'var(--brand-deep-red)' } : undefined} onMouseEnter={(e) => mainImage !== img && (e.currentTarget.style.borderColor = 'var(--brand-orange)')} onMouseLeave={(e) => mainImage !== img && (e.currentTarget.style.borderColor = '')}
        ...
        <div className="w-full md:w-1/2 p-6 flex flex-col overflow-y-auto">
          ...
          <div className="mt-auto pt-6 space-y-3">
             <button onClick={handleFullDetailsClick} className="w-full btn-brand-primary text-brand-white font-bold py-3 px-6 rounded-lg text-lg transition-all transform hover:scale-105">
                View Full Details
            </button>
            <div className="flex gap-3">
                <button
                    onClick={() => onToggleWishlist(vehicle.id)}
                    className={`w-full font-bold py-3 px-6 rounded-lg text-lg transition-all flex items-center justify-center gap-2 ${isInWishlist ? 'bg-brand-orange text-brand-deep-red' : 'bg-brand-gray-200 dark:bg-brand-gray-700'}`}
                >
                 ...
                 <button
                    ...
                    className={`w-full font-bold py-3 px-6 rounded-lg text-lg transition-all ${isComparing ? 'bg-brand-deep-red text-brand-deep-red' : 'bg-brand-gray-200 dark:bg-brand-gray-700'} ${isCompareDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                    Compare
                </button>
            </div>

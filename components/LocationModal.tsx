        ...
        <div className="fixed inset-0 bg-black/60 z-[101] flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
            <div className="bg-brand-white dark:bg-brand-gray-800 rounded-xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b dark:border-gray-700">
                    <h2 className="text-xl font-bold text-brand-blackcurrant dark:text-brand-blackcurrant">Select Your Location</h2>
                    <p className="text-sm text-brand-blackcurrant dark:text-brand-blackcurrant mt-1">Prices and availability may vary based on your location.</p>
                    {currentLocation && (
                        <div className="mt-3 flex items-center gap-2 text-sm">
                            ...
                            <span className="text-brand-blackcurrant dark:text-brand-blackcurrant">Current: <strong style={{ color: 'var(--brand-deep-red)' }}>{currentLocation}</strong></span>
                        </div>
                    )}
                </div>
                <div className="p-6 space-y-4">
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 font-bold py-3 px-4 rounded-lg transition-colors disabled:opacity-70 disabled:cursor-wait" style={{ background: 'var(--brand-orange-light)', color: 'var(--brand-blackcurrant)' }} onMouseEnter={(e) => !isDetecting && (e.currentTarget.style.background = 'var(--brand-orange)')} onMouseLeave={(e) => !isDetecting && (e.currentTarget.style.background = 'var(--brand-orange-light)')}
                    >
                        {isDetecting ? (
                            <>
                    ...
                    <div className="flex items-center text-xs text-brand-blackcurrant">
                        <span className="flex-grow border-t dark:border-gray-600"></span>
                        <span className="px-4">OR</span>
                        <span className="flex-grow border-t dark:border-gray-600"></span>
                    </div>

                    <p className="text-sm font-semibold text-center text-brand-blackcurrant dark:text-brand-blackcurrant">Select your location manually</p>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="state-select" className="block text-sm font-medium text-brand-blackcurrant dark:text-brand-blackcurrant mb-1">State</label>
                            <select id="state-select" value={selectedState} onChange={e => { setSelectedState(e.target.value); setSelectedCity(''); }} className="w-full p-2 border border-brand-orange dark:border-gray-600 rounded-md bg-brand-white dark:bg-brand-gray-700">
                                <option value="" disabled>Select a state</option>
                                {INDIAN_STATES.map(state => <option key={state.code} value={state.code}>{state.name}</option>)}
                            </select>
                        </div>
                        <div>
                            <label htmlFor="city-select" className="block text-sm font-medium text-brand-blackcurrant dark:text-brand-blackcurrant mb-1">City</label>
                            <select id="city-select" value={selectedCity} onChange={e => setSelectedCity(e.target.value)} disabled={!selectedState || availableCities.length === 0} className="w-full p-2 border border-brand-orange dark:border-gray-600 rounded-md bg-brand-white dark:bg-brand-gray-700 disabled:bg-brand-white dark:disabled:bg-brand-white">
                ...
                <div className="bg-brand-white dark:bg-brand-gray-darker px-6 py-4 flex justify-end gap-4 rounded-b-xl">
                    ...
                    <button onClick={handleManualSelect} disabled={!selectedCity} className="px-4 py-2 btn-brand-primary text-brand-white rounded-md disabled:opacity-50">Set Location</button>
                </div>
            </div>
        </div>
    );

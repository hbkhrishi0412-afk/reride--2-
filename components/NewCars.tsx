...
const CarModelCard: React.FC<{ car: NewCarModel; isExpanded: boolean; onToggle: (id: number) => void; selectedState: string; onViewSpecs: (car: NewCarModel, variant: NewCarVariant) => void; }> = ({ car, isExpanded, onToggle, selectedState, onViewSpecs }) => (
    <div className="bg-white rounded-xl shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-soft-lg">
        <button
            ...
            className="w-full p-4 text-left flex items-center gap-4 focus:outline-none rounded-xl" onFocus={(e) => e.currentTarget.style.boxShadow = '0 0 0 2px var(--brand-turquoise-light)'} onBlur={(e) => e.currentTarget.style.boxShadow = ''}
        >
            ...
            <div className="flex-grow">
                <h3 className="text-xl font-bold text-brand-gray-900 dark:text-brand-text-dark">{car.brand_name} {car.model_name} ({car.model_year})</h3>
                <p className="text-sm text-brand-gray-600 dark:text-brand-text">{car.body_type} &middot; {car.fuel_options.join(', ')}</p>
                
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-brand-text dark:text-brand-text">
                ...
                <p className="text-lg font-semibold mt-2" style={{ color: 'var(--brand-turquoise)' }}>
                ...
                <div className="border-t border-brand-gray-200 dark:border-brand-gray-200 pt-4 overflow-x-auto">
                    <table className="min-w-full divide-y divide-brand-gray-200 dark:divide-brand-gray-700">
                        <thead className="bg-white dark:bg-brand-gray-700">
                            <tr>
                                <th className="px-4 py-2 text-left text-xs font-medium text-brand-text dark:text-brand-text-dark uppercase">Variant</th>
                                <th className="px-4 py-2 text-left text-xs font-medium text-brand-text dark:text-brand-text-dark uppercase">On-Road Price ({selectedState})</th>
                                <th className="px-4 py-2 text-left text-xs font-medium text-brand-text dark:text-brand-text-dark uppercase">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-brand-gray-200 dark:divide-brand-gray-700">
                            {car.variants.map(variant => (
                                <tr key={variant.variant_name}>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-brand-gray-900 dark:text-brand-text-dark">{variant.variant_name} ({variant.transmission})</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm font-semibold text-brand-text-dark dark:text-brand-gray-200">{formatCurrency(variant.on_road_prices[selectedState])}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">
                                        <button onClick={() => onViewSpecs(car, variant)} className="font-semibold hover:underline transition-colors" style={{ color: 'var(--brand-turquoise)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-mint)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-turquoise)'}>
                                            View Full Specs
                                        </button>
                                    </td>
                                </tr>
...
const FilterSelect: React.FC<{ label: string; value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; children: React.ReactNode, disabled?: boolean }> = ({ label, value, onChange, children, disabled = false }) => (
    <div>
        <label className="block text-sm font-medium text-brand-text-dark dark:text-brand-text-dark mb-1">{label}</label>
        <select value={value} onChange={onChange} disabled={disabled} className="w-full p-3 border border-brand-gray-300 dark:border-brand-gray-300 rounded-lg focus:outline-none bg-brand-white dark:bg-brand-gray-700 text-brand-text-dark dark:text-brand-gray-200 disabled:opacity-50 disabled:cursor-not-allowed" onFocus={(e) => !disabled && (e.currentTarget.style.boxShadow = '0 0 0 2px var(--brand-turquoise-light)')} onBlur={(e) => e.currentTarget.style.boxShadow = ''}>
            {children}
        </select>
    </div>
);
    ...
    return (
        ...
        <div className="new-cars container mx-auto px-4 py-8 animate-fade-in">
            <div className="text-center mb-12">
                <h1 className="text-4xl md:text-5xl font-extrabold text-brand-text-dark dark:text-brand-text-dark mb-4">
                    Discover New Cars in India
                </h1>
                <p className="text-lg text-brand-gray-600 dark:text-brand-text max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8">
                ...
                <aside className="hidden lg:block self-start sticky top-24">
                    <div className="bg-white p-6 rounded-xl shadow-soft-lg space-y-4">
                        ...
                        <button onClick={handleClearFilters} className="w-full mt-2 bg-brand-gray-200 dark:bg-brand-gray-700 text-brand-text-dark dark:text-brand-gray-200 font-bold py-3 px-4 rounded-lg hover:bg-brand-gray-300 dark:hover:bg-brand-gray-600 transition-colors">
                            Clear All Filters
                        </button>
                    </div>
                </aside>
                ...
                <main className="space-y-6">
                    <input
                        ...
                        className="w-full p-4 border border-brand-gray-300 dark:border-brand-gray-300 rounded-xl focus:outline-none bg-brand-white dark:bg-brand-gray-700 text-brand-text-dark dark:text-brand-gray-200 text-lg" onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--brand-turquoise)'; e.currentTarget.style.boxShadow = '0 0 0 2px var(--brand-turquoise-light)'; }} onBlur={(e) => { e.currentTarget.style.borderColor = ''; e.currentTarget.style.boxShadow = ''; }}
                    />
                    <div className="flex justify-between items-center">
                        <button onClick={handleOpenFilterModal} className="lg:hidden flex items-center gap-2 font-semibold px-4 py-2 rounded-lg" style={{ color: 'var(--brand-turquoise)', background: 'var(--brand-mint-light)' }} onMouseEnter={(e) => e.currentTarget.style.background = 'var(--brand-mint)'} onMouseLeave={(e) => e.currentTarget.style.background = 'var(--brand-mint-light)'}>
                        ...
                        <p className="text-sm text-brand-gray-600 dark:text-brand-text">
                            Showing <span className="font-bold text-brand-text-dark dark:text-brand-gray-200">{filteredCars.length}</span> of <span className="font-bold text-brand-text-dark dark:text-brand-gray-200">{carModels.length}</span> models
                        </p>
                    </div>
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        {isLoading ? (
                            ...
                            <div className="xl:col-span-2 text-center py-16 bg-white rounded-xl shadow-soft">
                                <h3 className="text-2xl font-semibold text-brand-text-dark dark:text-brand-gray-200">No Cars Found</h3>
                                <p className="text-brand-text dark:text-brand-text mt-2">
            ...
            <div className="lg:hidden fixed inset-0 bg-black bg-opacity-60 z-50 animate-fade-in" onClick={() => setIsFilterModalOpen(false)}>
                <div className="bg-white rounded-t-2xl h-[90vh] flex flex-col absolute bottom-0 left-0 right-0 animate-slide-in-up" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b border-brand-gray-200 dark:border-brand-gray-200 flex justify-between items-center flex-shrink-0">
                        <h2 className="text-xl font-bold text-brand-text-dark dark:text-brand-text-dark">Filters</h2>
                        <button onClick={() => setIsFilterModalOpen(false)} className="p-2 -mr-2 text-brand-text text-2xl">&times;</button>
                    </div>

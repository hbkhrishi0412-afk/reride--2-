...
const badgeStyles: Record<Badge['type'], { icon: React.ReactElement<{ className?: string }>; colors: string }> = {
    verified: {
        ...
        colors: 'bg-gradient-primary text-brand-white border-brand-deep-red',
    },
    top_seller: {
        ...
        colors: 'bg-brand-orange text-brand-blackcurrant dark:bg-brand-orange dark:text-brand-blackcurrant border-amber-500/50',
    },
    high_rating: {
        ...
        colors: 'bg-brand-deep-red-light text-brand-deep-red dark:bg-brand-deep-red dark:text-brand-deep-red border-brand-deep-red/50',
    }
};
        ...
        <div className="flex flex-wrap gap-2 items-center">
            {badges.map(badge => (
                <div key={badge.type} className="group relative">
                    ...
                    <div className="absolute bottom-full mb-2 w-60 bg-brand-gray-900 text-brand-white text-xs rounded py-1.5 px-2.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10 left-1/2 -translate-x-1/2">
                        {badge.description}
                    </div>
                </div>
            ))}

...
const badgeStyles: Record<Badge['type'], { icon: React.ReactElement<{ className?: string }>; colors: string }> = {
    verified: {
        ...
        colors: 'bg-gradient-primary text-brand-cream border-brand-teal',
    },
    top_seller: {
        ...
        colors: 'bg-brand-gold text-brand-slate dark:bg-brand-gold dark:text-brand-slate border-amber-500/50',
    },
    high_rating: {
        ...
        colors: 'bg-brand-teal-light text-brand-teal dark:bg-brand-teal dark:text-brand-teal border-brand-teal/50',
    }
};
        ...
        <div className="flex flex-wrap gap-2 items-center">
            {badges.map(badge => (
                <div key={badge.type} className="group relative">
                    ...
                    <div className="absolute bottom-full mb-2 w-60 bg-brand-gray-900 text-brand-cream text-xs rounded py-1.5 px-2.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10 left-1/2 -translate-x-1/2">
                        {badge.description}
                    </div>
                </div>
            ))}

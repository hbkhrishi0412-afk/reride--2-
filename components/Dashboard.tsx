...
const HelpTooltip: React.FC<{ text: string }> = memo(({ text }) => (
    <span className="group relative ml-1">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-brand-slate cursor-help" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        <span className="absolute bottom-full mb-2 w-48 bg-brand-cream text-brand-cream text-xs rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 left-1/2 -translate-x-1/2 z-10">{text}</span>
    </span>
));
...
const FormInput: React.FC<{ label: string; name: keyof Vehicle | 'summary'; type?: string; value: string | number; onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => void; onBlur?: (e: React.FocusEvent<HTMLInputElement>) => void; error?: string; tooltip?: string; required?: boolean; children?: React.ReactNode; disabled?: boolean; placeholder?: string; rows?: number }> = 
  ...
  <div>
    <label htmlFor={String(name)} className="flex items-center text-sm font-medium text-brand-slate dark:text-brand-slate mb-1">
        {label}{required && <span className="text-brand-teal ml-0.5">*</span>}
        {tooltip && <HelpTooltip text={tooltip} />}
    </label>
    {type === 'select' ? (
        <select id={String(name)} name={String(name)} value={String(value)} onChange={onChange} required={required} disabled={disabled} className={`block w-full p-3 border rounded-lg focus:outline-none transition bg-brand-cream dark:bg-brand-gray-darker dark:text-brand-slate disabled:bg-brand-cream dark:disabled:bg-brand-cream ${error ? 'border-brand-teal' : 'border-brand-gray dark:border-gray-600'}`} onFocus={(e) => !error && (e.currentTarget.style.boxShadow = '0 0 0 3px var(--brand-teal-light)')} onBlur={(e) => e.currentTarget.style.boxShadow = ''}>
            {children}
        </select>
    ) : type === 'textarea' ? (
        <textarea id={String(name)} name={String(name)} value={String(value)} onChange={onChange} required={required} disabled={disabled} placeholder={placeholder} rows={rows} className={`block w-full p-3 border rounded-lg focus:outline-none transition bg-brand-cream dark:bg-brand-gray-darker dark:text-brand-slate disabled:bg-brand-cream dark:disabled:bg-brand-cream ${error ? 'border-brand-teal' : 'border-brand-gray dark:border-gray-600'}`} onFocus={(e) => !error && (e.currentTarget.style.boxShadow = '0 0 0 3px var(--brand-teal-light)')} onBlur={(e) => e.currentTarget.style.boxShadow = ''} />
    ) : (
        <input type={type} id={String(name)} name={String(name)} value={value} onChange={onChange} required={required} disabled={disabled} placeholder={placeholder} className={`block w-full p-3 border rounded-lg focus:outline-none transition bg-brand-cream dark:bg-brand-gray-darker dark:text-brand-slate disabled:bg-brand-cream dark:disabled:bg-brand-cream ${error ? 'border-brand-teal' : 'border-brand-gray dark:border-gray-600'}`} onFocus={(e) => !error && (e.currentTarget.style.boxShadow = '0 0 0 3px var(--brand-teal-light)')} onBlur={(e) => { e.currentTarget.style.boxShadow = ''; if (onBlur) onBlur(e); }} />
    )}
    {error && <p className="mt-1 text-xs text-brand-teal">{error}</p>}
  </div>
);


const StatCard: React.FC<{ title: string; value: string | number; icon: React.ReactNode }> = memo(({ title, value, icon }) => (
  <div className="bg-brand-cream dark:bg-brand-gray-dark p-6 rounded-lg shadow-md flex items-center">
...
const PlanStatusCard: React.FC<{
    ...
    return (
        <div className="text-brand-cream p-6 rounded-lg shadow-lg flex flex-col h-full" style={{ background: 'var(--gradient-primary)' }}>
            <h3 className="text-lg font-bold flex justify-between items-center">
                <span>Your Plan: <span className="text-brand-slate">{plan.name}</span></span>
            </h3>
            <div className="mt-4 space-y-3 text-sm flex-grow">
                ...
                <div className="w-full rounded-full h-2 mb-2" style={{ background: 'var(--brand-gold-light)' }}>
                    <div
                    ...
                    <ul className="space-y-2 text-xs">
                        {plan.features.map(feature => (
                            <li key={feature} className="flex items-start">
                                <svg className="w-4 h-4 text-brand-teal mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            ...
            {plan.id !== 'premium' && (
                <button
                    ...
                    className="mt-6 w-full bg-brand-cream text-brand-teal font-bold py-2 px-4 rounded-lg hover:bg-brand-cream transition-colors"
                >
                    Upgrade Plan
                </button>
            )}
...
const FormFieldset: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => {
    ...
    return (
        <fieldset className="border border-brand-gold dark:border-gray-700 rounded-lg p-4">
            <legend className="px-2 text-lg font-semibold text-brand-slate dark:text-brand-slate">
...
const VehicleForm: React.FC<VehicleFormProps> = memo(({ editingVehicle, onAddVehicle, onUpdateVehicle, onCancel, vehicleData, seller, onFeatureListing, allVehicles }) => {
    ...
    return (
      <div className="bg-brand-cream dark:bg-brand-gray-dark p-6 sm:p-8 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold text-brand-slate dark:text-brand-slate mb-6 border-b dark:border-gray-700 pb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <FormFieldset title="Vehicle Overview">
                <div className="flex justify-between items-center mb-4 -mt-4">
                    <p className="text-xs text-brand-slate dark:text-brand-slate">Enter core details about your vehicle.</p>
                    <button type="button" onClick={handleGetAiSuggestions} disabled={isGeneratingSuggestions || !formData.make || !formData.model || !formData.year} className="text-sm font-semibold text-brand-teal disabled:opacity-50 flex items-center gap-1">
            ...
            <FormFieldset title="Vehicle Specifications">
                 <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4">
                    <div>
                        ...
                        {aiSuggestions?.structuredSpecs.engine && formData.engine !== aiSuggestions.structuredSpecs.engine && (<button type="button" onClick={() => applyAiSpec('engine')} className="text-xs text-brand-teal hover:underline mt-1">Apply: "{aiSuggestions.structuredSpecs.engine}"</button>)}
                    </div>
                    <div>
                        <FormInput label="Displacement" name="displacement" value={formData.displacement} onChange={handleChange} placeholder="e.g., 1497 cc"/>
                        {aiSuggestions?.structuredSpecs.displacement && formData.displacement !== aiSuggestions.structuredSpecs.displacement && (<button type="button" onClick={() => applyAiSpec('displacement')} className="text-xs text-brand-teal hover:underline mt-1">Apply: "{aiSuggestions.structuredSpecs.displacement}"</button>)}
                    </div>
                     <div>
                        ...
                        {aiSuggestions?.structuredSpecs.transmission && formData.transmission !== aiSuggestions.structuredSpecs.transmission && (<button type="button" onClick={() => applyAiSpec('transmission')} className="text-xs text-brand-teal hover:underline mt-1">Apply: "{aiSuggestions.structuredSpecs.transmission}"</button>)}
                    </div>
                    <div>
                        <FormInput label="Fuel Type" name="fuelType" type="select" value={formData.fuelType} onChange={handleChange}>
                         ...
                         {aiSuggestions?.structuredSpecs.fuelType && formData.fuelType !== aiSuggestions.structuredSpecs.fuelType && (<button type="button" onClick={() => applyAiSpec('fuelType')} className="text-xs text-brand-teal hover:underline mt-1">Apply: "{aiSuggestions.structuredSpecs.fuelType}"</button>)}
                    </div>
                    <div>
                        <FormInput label="Mileage / Range" name="fuelEfficiency" value={formData.fuelEfficiency} onChange={handleChange} tooltip="e.g., 18 KMPL or 300 km range"/>
                         {aiSuggestions?.structuredSpecs.fuelEfficiency && formData.fuelEfficiency !== aiSuggestions.structuredSpecs.fuelEfficiency && (<button type="button" onClick={() => applyAiSpec('fuelEfficiency')} className="text-xs text-brand-teal hover:underline mt-1">Apply: "{aiSuggestions.structuredSpecs.fuelEfficiency}"</button>)}
                    </div>
                     <div>
                        ...
                        {aiSuggestions?.structuredSpecs.groundClearance && formData.groundClearance !== aiSuggestions.structuredSpecs.groundClearance && (<button type="button" onClick={() => applyAiSpec('groundClearance')} className="text-xs text-brand-teal hover:underline mt-1">Apply: "{aiSuggestions.structuredSpecs.groundClearance}"</button>)}
                    </div>
                    <div>
                        <FormInput label="Boot Space" name="bootSpace" value={formData.bootSpace} onChange={handleChange} placeholder="e.g., 433 litres"/>
                        {aiSuggestions?.structuredSpecs.bootSpace && formData.bootSpace !== aiSuggestions.structuredSpecs.bootSpace && (<button type="button" onClick={() => applyAiSpec('bootSpace')} className="text-xs text-brand-teal hover:underline mt-1">Apply: "{aiSuggestions.structuredSpecs.bootSpace}"</button>)}
                    </div>
            ...
            <FormFieldset title="Quality Report">
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-brand-slate dark:text-brand-slate mb-1">Fixes Done / Upgrades</label>
                        <div className="flex gap-2">
                            ...
                            <button type="button" onClick={handleAddFix} className="bg-brand-cream-dark dark:bg-brand-cream font-bold py-2 px-4 rounded-lg">Add Fix</button>
                        </div>
                        <div className="mt-2 flex flex-wrap gap-2">
                            {(formData.qualityReport?.fixesDone || []).map(fix => (
                                <span key={fix} className="bg-brand-teal-light text-brand-teal text-sm font-semibold px-3 py-1 rounded-full flex items-center gap-2">
                                    {fix}
            ...
            <FormFieldset title="Media, Documents & Description">
                <div className="space-y-4">
                     <div>
                        <label className="block text-sm font-medium text-brand-slate dark:text-brand-slate">Images</label>
                        <div className="mt-1">
                            <label htmlFor="file-upload" className={`relative cursor-pointer bg-brand-cream dark:bg-brand-gray-darker rounded-lg border-2 border-brand-gold dark:border-gray-600 border-dashed transition-colors duration-200 flex flex-col items-center justify-center text-center p-6 ${isUploading ? 'opacity-50 cursor-not-allowed' : ''}`} style={{ borderColor: isUploading ? '' : undefined }} onMouseEnter={(e) => !isUploading && (e.currentTarget.style.borderColor = 'var(--brand-teal)')} onMouseLeave={(e) => !isUploading && (e.currentTarget.style.borderColor = '')}>
                                <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-brand-slate" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                                <span className="mt-2 block text-sm font-semibold" style={{ color: 'var(--brand-teal)' }}>
                        ...
                        <div className="mt-4 grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-3">
                            {formData.images.map((url, index) => (
                                <div key={index} className="relative group">
                                    ...
                                    <button type="button" onClick={() => handleRemoveImageUrl(url)} className="absolute top-1 right-1 bg-brand-teal text-brand-cream rounded-full h-6 w-6 flex items-center justify-center text-sm opacity-0 group-hover:opacity-100">&times;</button>
                                </div>
                            ))}
                        </div>
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-brand-slate dark:text-brand-slate">Documents</label>
                        <div className="mt-1 flex items-center gap-2">
                             <select id="document-type" className="p-3 border rounded-lg bg-brand-cream dark:bg-brand-gray-darker dark:text-brand-slate border-brand-gray dark:border-gray-600">
                            ...
                            <label htmlFor="doc-upload" className={`cursor-pointer font-semibold text-brand-cream py-2 px-4 rounded-lg transition-colors ${isUploading ? 'bg-brand-cream' : 'btn-brand-primary'}`} style={!isUploading ? { background: 'var(--gradient-primary)', cursor: 'pointer' } : undefined}>
                                {isUploading ? '...' : 'Upload'}
                         ...
                         <div className="mt-2 flex flex-wrap gap-2">
                            {(formData.documents || []).map(doc => (
                                <span key={doc.url} className="bg-brand-cream-dark dark:bg-brand-cream text-sm font-semibold px-3 py-1 rounded-full flex items-center gap-2">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-brand-slate dark:text-brand-slate mb-1">Key Features</label>
                        <div className="flex gap-2">
                            ...
                            <button type="button" onClick={handleAddFeature} className="bg-brand-cream-dark dark:bg-brand-cream font-bold py-2 px-4 rounded-lg">Add</button>
                        </div>
                     ...
                     <div>
                        <div className="flex justify-between items-center mb-1">
                            ...
                            <button type="button" onClick={handleGenerateDescription} disabled={isGeneratingDesc || !formData.make || !formData.model} className="text-sm font-semibold text-brand-teal disabled:opacity-50"> {isGeneratingDesc ? '...' : '✨ Generate with AI'}</button>
                        </div>
            ...
            <FormFieldset title="Promotion">
                {(!editingVehicle || !editingVehicle.isFeatured) && (
                    <div className="p-4 bg-brand-teal dark:bg-brand-teal/20 border border-brand-teal dark:border-brand-teal rounded-lg">
                        <div className="flex items-center justify-between">
                            <div>
                                <label htmlFor="feature-listing" className="font-bold text-brand-teal dark:text-brand-teal flex items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                                    Feature this Listing
                                </label>
                                <p className="text-xs text-brand-teal dark:text-brand-teal mt-1">
                            ...
                            <input
                                ...
                                className="h-6 w-6 text-brand-teal bg-brand-cream border-brand-gold rounded focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed"
                            />
                        </div>
                        {(seller.featuredCredits || 0) <= 0 && <p className="text-xs text-brand-teal mt-2">You have no featured credits. Upgrade your plan to get more.</p>}
                    </div>
                )}
            </FormFieldset>
            ...
            <div className="pt-4 flex flex-col sm:flex-row items-center gap-4">
                ...
                <button type="button" onClick={onCancel} className="w-full sm:w-auto bg-brand-cream0 text-brand-cream font-bold py-3 px-6 rounded-lg text-lg hover:bg-brand-cream">Cancel</button>
            </div>
          </form>
          ...
          <div className="hidden lg:block">
              <div className="sticky top-24 self-start space-y-6">
                  <div>
                      <h3 className="text-lg font-semibold text-brand-slate dark:text-brand-slate mb-4">Live Preview</h3>
                   ...
                   {aiSuggestions && Object.keys(aiSuggestions.featureSuggestions).length > 0 && (
                     <div>
                        <h3 className="text-lg font-semibold text-brand-slate dark:text-brand-slate mb-4 flex items-center gap-2">✨ Suggested Features</h3>
                            ...
                            {Object.entries(aiSuggestions.featureSuggestions).map(([category, features]) => {
                                ...
                                return (
                                    <div key={category} className="mb-4 last:mb-0">
                                        <h4 className="font-bold text-brand-slate dark:text-brand-slate mb-2 pb-1 border-b dark:border-gray-600">{category}</h4>
                                        <div className="space-y-2">
                                            {features.map(feature => (
                                                <label key={feature} className="flex items-center space-x-3 cursor-pointer group">
                                                    <input type="checkbox" checked={formData.features.includes(feature)} onChange={() => handleSuggestedFeatureToggle(feature)} className="h-4 w-4 rounded border-brand-gold dark:border-gray-500 bg-transparent" style={{ accentColor: 'var(--brand-teal)' }} />
                                                    <span className="text-sm text-brand-slate dark:text-brand-slate transition-colors" onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-teal)'} onMouseLeave={(e) => e.currentTarget.style.color = ''}>{feature}</span>
                                                </label>
                                            ))}
                                        </div>
                                    </div>
...
const InquiriesView: React.FC<{
  ...
  onSellerSendMessage: (conversationId: string, messageText: string) => void;
    ...
    return (
       <div className="bg-brand-cream dark:bg-brand-gray-dark p-6 sm:p-8 rounded-lg shadow-md">
         <h2 className="text-2xl font-bold text-brand-slate dark:text-brand-slate mb-6">Customer Inquiries</h2>
         <div className="space-y-2">
            {sortedConversations.length > 0 ? sortedConversations.map(conv => (
              <div key={conv.id} onClick={() => handleSelectConversation(conv)} className="p-4 rounded-lg cursor-pointer hover:bg-brand-gray-light dark:hover:bg-brand-gray-darker border-b dark:border-gray-700 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    {!conv.isReadBySeller && <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: 'var(--brand-teal)' }}></div>}
                    <div>
                      <p className="font-bold text-brand-slate dark:text-brand-slate">{conv.customerName} - <span className="font-normal text-brand-slate dark:text-brand-slate">{conv.vehicleName}</span></p>
                      <p className="text-sm text-brand-slate dark:text-brand-slate truncate max-w-md">{conv.messages[conv.messages.length - 1].text}</p>
                    </div>
                </div>
                <span className="text-xs text-brand-slate dark:text-brand-slate">{new Date(conv.lastMessageAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
              </div>
            )) : (
                <div className="text-center py-16 px-6">
                    ...
                    <h3 className="mt-2 text-xl font-semibold text-brand-slate dark:text-brand-slate">No inquiries yet</h3>
                    <p className="mt-1 text-sm text-brand-slate dark:text-brand-slate">When a customer sends a message about one of your listings, it will appear here.</p>
                </div>
            )}
         </div>
       </div>
...
const ProfileInput: React.FC<{ label: string; name: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; }> = ({ label, name, value, onChange }) => (
    <div>
        <label htmlFor={name} className="block text-sm font-medium text-brand-slate dark:text-brand-slate">{label}</label>
        <input
            type="text"
            name={name}
            id={name}
            value={value}
            onChange={onChange}
            className="mt-1 block w-full p-3 border border-brand-gold dark:border-gray-600 rounded-lg shadow-sm sm:text-sm bg-brand-cream dark:bg-brand-gray-darker dark:text-brand-slate" onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--brand-teal)'; e.currentTarget.style.boxShadow = '0 0 0 2px var(--brand-teal-light)'; }} onBlur={(e) => { e.currentTarget.style.borderColor = ''; e.currentTarget.style.boxShadow = ''; }}
        />
    </div>
);
...
const SellerProfileForm: React.FC<{ seller: User; onUpdateProfile: (details: any) => void; }> = memo(({ seller, onUpdateProfile }) => {
    ...
    return (
        <div className="bg-brand-cream dark:bg-brand-gray-dark p-6 sm:p-8 rounded-lg shadow-md">
            <h2 className="text-2xl font-bold text-brand-slate dark:text-brand-slate mb-6">Seller Profile</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="flex items-center gap-6">
                    <img src={formData.logoUrl || 'https://via.placeholder.com/100'} alt="Logo" className="w-24 h-24 rounded-full object-cover bg-brand-cream-dark" />
                    <div>
                        <label htmlFor="logo-upload" className="cursor-pointer font-medium transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}>
                            <span>Upload New Logo</span>
                            <input id="logo-upload" name="logo-upload" type="file" className="sr-only" accept="image/*" onChange={handleLogoUpload} />
                        </label>
                        <p className="text-xs text-brand-slate mt-1">PNG, JPG, GIF up to 2MB.</p>
                    </div>
                </div>
                <ProfileInput 
                ...
                />
                <div>
                    <button type="submit" className="btn-brand-primary text-brand-cream font-bold py-3 px-6 rounded-lg">Save Profile</button>
                </div>
            </form>

             <div className="mt-8 pt-6 border-t border-brand-gold dark:border-gray-700">
                <h3 className="text-xl font-bold text-brand-slate dark:text-brand-slate mb-4">Your Public Profile</h3>
                <div className="flex flex-col sm:flex-row items-center gap-6 p-4 bg-brand-cream dark:bg-brand-gray-darker rounded-lg">
                    <div className="text-center">
                        <img src={qrCodeUrl} alt="Seller Profile QR Code" className="w-36 h-36 rounded-lg border dark:border-gray-600" />
                         <a href={qrCodeUrl} download={`qr-code-${seller.email}.png`} className="mt-2 inline-block text-sm hover:underline transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}>
                            Download QR Code
                        </a>
                    </div>
                    <div>
                        <p className="text-sm text-brand-slate dark:text-brand-slate mb-3">Share this QR code or link with customers. When they scan or click it, they'll be taken directly to your profile page with all your listings.</p>
                        <div className="mt-2 p-2 bg-brand-cream-dark dark:bg-brand-cream rounded-md flex items-center justify-between gap-2">
                           <a href={profileUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-mono truncate hover:underline transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}>{profileUrl}</a>
                           <button onClick={handleCopy} className="text-sm font-semibold px-3 py-1 text-brand-cream rounded-md transition-colors whitespace-nowrap btn-brand-primary">
...
const ReportsView: React.FC<{
    ...
    <div className="bg-brand-cream dark:bg-brand-gray-dark p-6 sm:p-8 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold text-brand-slate dark:text-brand-slate mb-6">Reported Listings</h2>
        {reportedVehicles.length > 0 ? (
            <div className="space-y-4">
                {reportedVehicles.map(v => (
                    <div key={v.id} className="border border-brand-gold dark:border-brand-gold bg-brand-gold-light dark:bg-brand-gold/20 p-4 rounded-lg">
                        <h3 className="font-bold text-brand-slate dark:text-brand-slate">{v.year} {v.make} {v.model}</h3>
                        <p className="text-sm text-brand-slate dark:text-brand-slate mt-1">Reported on: {v.flaggedAt ? new Date(v.flaggedAt).toLocaleString() : 'N/A'}</p>
                        <p className="mt-2 text-sm italic text-brand-slate dark:text-brand-slate">Reason: "{v.flagReason || 'No reason provided.'}"</p>
                        <p className="text-xs text-brand-slate dark:text-brand-slate mt-2">An administrator will review this report. You can edit the listing to correct any issues or delete it if it's no longer valid.</p>
                        <div className="mt-3 space-x-4">
                            <button onClick={() => onEditVehicle(v)} className="font-semibold text-sm hover:underline transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}> Edit Listing</button>
                            <button onClick={() => onDeleteVehicle(v.id)} className="text-brand-teal font-semibold text-sm hover:underline">Delete Listing</button>
                        </div>
                    </div>
                ))}
            </div>
             ...
             <div className="text-center py-16 px-6">
                <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-brand-slate" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <h3 className="mt-2 text-xl font-semibold text-brand-slate dark:text-brand-slate">All Clear!</h3>
                <p className="mt-1 text-sm text-brand-slate dark:text-brand-slate">You have no reported listings at this time.</p>
            </div>
        )}
    </div>
));
...
const Dashboard: React.FC<DashboardProps> = ({ seller, sellerVehicles, reportedVehicles, onAddVehicle, onAddMultipleVehicles, onUpdateVehicle, onDeleteVehicle, onMarkAsSold, conversations, onSellerSendMessage, onMarkConversationAsReadBySeller, typingStatus, onUserTyping, onMarkMessagesAsRead, onUpdateSellerProfile, vehicleData, onFeatureListing, onRequestCertification, onNavigate, onTestDriveResponse, allVehicles, onOfferResponse }) => {
      ...
      datasets: [
        {
          ...
          backgroundColor: 'rgba(52, 101, 109, 0.5)',
        ...
        },
        {
          ...
          borderColor: 'rgba(250, 234, 177, 1)',
          borderWidth: 1,
          yAxisID: 'y1',
        },
      ],
  ...
  const getCertificationButton = (vehicle: Vehicle) => {
      ...
      switch (status) {
          case 'requested':
              return <button disabled className="text-brand-slate text-sm font-semibold">Pending Approval</button>;
          case 'approved':
              return <span className="text-brand-teal text-sm font-semibold">Certified</span>;
          case 'rejected':
              return <button onClick={() => onRequestCertification(vehicle.id)} className="text-brand-teal hover:text-brand-teal" title="Certification was rejected, you can request again.">Request Again</button>;
          case 'none':
          default:
  ...
  const renderContent = () => {
    switch(activeView) {
      ...
      case 'analytics':
        return (
            <div className="space-y-6">
                ...
                <div className="bg-brand-cream dark:bg-brand-gray-dark p-6 sm:p-8 rounded-lg shadow-md">
                    <h2 className="text-2xl font-bold text-brand-slate dark:text-brand-slate mb-6">Listing Performance</h2>
                    {activeListings.length > 0 ? (
                        ...
                        <div className="text-center py-16 px-6">
                            <h3 className="mt-2 text-xl font-semibold text-brand-slate dark:text-brand-slate">No Data to Display</h3>
                            <p className="mt-1 text-sm text-brand-slate dark:text-brand-slate">Add a vehicle to see performance data.</p>
                        </div>
                    )}
                </div>
            </div>
        ...
        return (
          <div className="bg-brand-cream dark:bg-brand-gray-dark p-6 sm:p-8 rounded-lg shadow-md">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
              <h2 className="text-2xl font-bold text-brand-slate dark:text-brand-slate">Active Listings</h2>
              <div className="flex gap-2">
                <button onClick={() => setIsBulkUploadOpen(true)} className="bg-brand-teal text-brand-cream font-bold py-2 px-4 rounded-lg hover:bg-brand-teal">Bulk Upload</button>
                <button onClick={handleAddNewClick} className="btn-brand-primary text-brand-cream font-bold py-2 px-4 rounded-lg">List New Vehicle</button>
              </div>
            </div>
            {activeListings.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-brand-cream dark:bg-brand-cream"><tr><th className="px-6 py-3 text-left text-xs font-medium uppercase">Vehicle</th><th className="px-6 py-3 text-left text-xs font-medium uppercase">Price</th><th className="relative px-6 py-3 text-right text-xs font-medium uppercase">Actions</th></tr></thead>
                      ...
                      <tr key={v.id}>
                        ...
                        <td className="px-6 py-4 text-right whitespace-nowrap text-sm font-semibold space-x-3">
                          {!v.isFeatured && (seller.featuredCredits ?? 0) > 0 && (
                              <button onClick={() => onFeatureListing(v.id)} className="text-brand-teal hover:text-brand-teal" title="Use a credit to feature this listing">Feature</button>
                          )}
                          {getCertificationButton(v)}
                          <button onClick={() => onMarkAsSold(v.id)} className="text-brand-teal hover:text-brand-teal">Sold</button>
                          <button onClick={() => handleEditClick(v)} className="transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}>Edit</button>
                ...
                <div className="text-center py-12 bg-brand-cream dark:bg-brand-gray-darker rounded-lg border border-dashed border-brand-gold dark:border-gray-600">
                    <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-brand-slate" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2-2H5a2 2 0 01-2-2z" />
                    </svg>
                    <h3 className="mt-2 text-xl font-semibold text-brand-slate dark:text-brand-slate">No vehicles listed yet</h3>
                    <p className="mt-1 text-sm text-brand-slate dark:text-brand-slate">Ready to sell? Add your first vehicle to get started.</p>
                    <div className="mt-6">
                        <button
                            onClick={handleAddNewClick}
                            className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-brand-cream btn-brand-primary focus:outline-none focus:ring-2 focus:ring-offset-2" style={{ boxShadow: 'var(--shadow-red)' }}
                        >
      ...
      case 'salesHistory':
        return (
          <div className="bg-brand-cream dark:bg-brand-gray-dark p-6 sm:p-8 rounded-lg shadow-md">
            <h2 className="text-2xl font-bold text-brand-slate dark:text-brand-slate mb-6">Sales History</h2>
            {soldListings.length > 0 ? (
                <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-brand-cream dark:bg-brand-cream"><tr><th className="px-6 py-3 text-left text-xs font-medium uppercase">Vehicle</th><th className="px-6 py-3 text-left text-xs font-medium uppercase">Sold Price</th></tr></thead>
                  <tbody className="bg-brand-cream dark:bg-brand-gray-dark divide-y divide-gray-200 dark:divide-gray-700">
                    {soldListings.map((v) => (
                      <tr key={v.id}>
                        <td className="px-6 py-4 font-medium">{v.year} {v.make} {v.model} {v.variant || ''}</td>
                ...
                <p className="text-center text-brand-slate dark:text-brand-slate py-8">You have not sold any vehicles yet.</p>
            )}
          </div>
        );
      case 'form':
  ...
  const NavItem: React.FC<{ view: DashboardView, children: React.ReactNode, count?: number }> = ({ view, children, count }) => (
    <button onClick={() => handleNavigate(view)} className={`flex justify-between items-center w-full text-left px-4 py-3 rounded-lg transition-colors ${activeView === view ? 'text-brand-cream' : 'hover:bg-brand-gray-light dark:hover:bg-brand-gray-darker'}`} style={activeView === view ? { background: 'var(--gradient-primary)' } : undefined}>
      <span>{children}</span>
      {count && count > 0 && <span className="bg-brand-teal-light0 text-brand-cream text-xs font-bold rounded-full px-2 py-0.5">{count}</span>}
    </button>
  );
  ...
  return (
    <div className="container mx-auto py-8 animate-fade-in">
        <div className="grid grid-cols-1 md:grid-cols-[250px_1fr] gap-8">
            <aside>
            <div className="bg-brand-cream dark:bg-brand-gray-dark p-4 rounded-lg shadow-md space-y-2">

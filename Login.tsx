...
const Login: React.FC<LoginProps> = ({ onLogin, onRegister, onNavigate, onForgotPassword }) => {
  ...
  const formInputClass = "appearance-none relative block w-full px-4 py-3 border border-brand-gray-300 dark:border-brand-gray-600 placeholder-brand-gray-500 text-brand-gray-900 dark:text-brand-gray-200 bg-brand-cream dark:bg-brand-gray-800 focus:outline-none focus:ring-brand-blue focus:border-brand-blue focus:z-10 sm:text-sm";

  return (
    <div className="seller-dashboard-login w-full max-w-md space-y-8 bg-brand-cream dark:bg-brand-gray-800 p-10 rounded-xl shadow-soft-xl">
      <div>
        ...
        {isLogin && (
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <input id="remember-me" name="remember-me" type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="h-4 w-4 border-brand-gray-300 rounded" style={{ accentColor: 'var(--brand-teal)' }} />
            ...
            <div className="text-sm">
              <button type="button" onClick={onForgotPassword} className="font-medium transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}>Forgot your password?</button>
            </div>
          </div>
        )}

        {error && <p className="text-brand-teal text-sm text-center">{error}</p>}

        <div>
          <button type="submit" disabled={isLoading} className="btn-brand-primary group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-brand-cream focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors disabled:opacity-50">
            {isLoading ? 'Processing...' : (isLogin ? 'Sign in' : 'Create Account')}
          </button>
        </div>
      </form>
      <div className="text-sm text-center">
          <button onClick={toggleMode} className="font-medium transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}>
              {isLogin ? "Don't have a seller account? Register" : "Already have a seller account? Sign in"}
          </button>
      </div>
       <div className="text-center">
          <button onClick={() => onNavigate(View.LOGIN_PORTAL)} className="font-medium transition-colors" style={{ color: 'var(--brand-teal)' }} onMouseEnter={(e) => e.currentTarget.style.color = 'var(--brand-gold)'} onMouseLeave={(e) => e.currentTarget.style.color = 'var(--brand-teal)'}>
